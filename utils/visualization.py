...
BASE_COLORS = adjust_colors(BASE_COLORS, saturation_threshold=0.3, brightness_threshold=0.8)
...
def vis_scale_img(img, scale_map, conf_thresh=0.3, patch_size=14):
    ...
    for (i, j) in zip(loc_i.tolist(), loc_j.tolist()):
        cv2.rectangle(img, (j * patch_size, i * patch_size), ((j + 1) * patch_size, (i + 1) * patch_size), color=(255, 255, 255), thickness=2)
