...
import pdb
from models.hoi_detect.detr.util.box_ops import box_xyxy_to_cxcywh
from models.hoi_detect.detr.util.misc import interpolate
...
def crop(image, target, region):
    ...
    if "boxes" in target or "masks" in target or "boxes_h" in target or "boxes_o" in target or 'ex_bbox' in target:
        ...
        if "boxes" in target:
            ...
            keep = torch.logical_and(torch.all(cropped_bh[:, 1, :] > cropped_bo[:, 0, :], dim=1), torch.all(cropped_bo[:, 1, :] > cropped_bo[:, 0, :], dim=1))
