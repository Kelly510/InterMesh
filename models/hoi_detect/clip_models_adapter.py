...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory, tgt_mask: Optional[Tensor] = None, memory_mask: Optional[Tensor] = None, tgt_key_padding_mask: Optional[Tensor] = None, memory_key_padding_mask: Optional[Tensor] = None, pos: Optional[Tensor] = None, query_pos: Optional[Tensor] = None):
        ...
        # tgt = self.norm1(tgt)
        tgt2, attentions = self.multihead_attn(query=self.with_pos_embed(tgt, query_pos), key=self.with_pos_embed(memory, pos), value=memory, attn_mask=memory_mask, key_padding_mask=memory_key_padding_mask)
    ...
    def forward_pre(self, tgt, memory, tgt_mask: Optional[Tensor] = None, memory_mask: Optional[Tensor] = None, tgt_key_padding_mask: Optional[Tensor] = None, memory_key_padding_mask: Optional[Tensor] = None, pos: Optional[Tensor] = None, query_pos: Optional[Tensor] = None):
        ...
        # tgt = tgt + self.dropout1(tgt2)
        # tgt2 = self.norm2(tgt)
        tgt2, attentions = self.multihead_attn(query=self.with_pos_embed(tgt2, query_pos), key=self.with_pos_embed(memory, pos), value=memory, attn_mask=memory_mask, key_padding_mask=memory_key_padding_mask)
        tgt = tgt + self.dropout2(tgt2)
        tgt2 = self.norm3(tgt)
...
class TransformerDecoderFusionLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", normalize_before=False, enable_cp=False):
        super().__init__()
    ...
    def forward_post(self, tgt, memory, sup_memory=None, tgt_mask: Optional[Tensor] = None, memory_mask: Optional[Tensor] = None, tgt_key_padding_mask: Optional[Tensor] = None, memory_key_padding_mask: Optional[Tensor] = None, pos: Optional[Tensor] = None, query_pos: Optional[Tensor] = None):
        # q = k = self.with_pos_embed(tgt, query_pos)
        # tgt2 = self.self_attn(q, k, value=tgt, attn_mask=tgt_mask,
        #                         key_padding_mask=tgt_key_padding_mask)[0]
        # tgt = tgt + self.dropout1(tgt2)
        ...
        tgt3 = self.multihead_attn(query=self.with_pos_embed(tgt, query_pos), key=self.with_pos_embed(sup_memory, pos), value=sup_memory, attn_mask=memory_mask, key_padding_mask=memory_key_padding_mask)[0]
    ...
    def forward(self, tgt, memory, sup_memory=None, tgt_mask: Optional[Tensor] = None, memory_mask: Optional[Tensor] = None, tgt_key_padding_mask: Optional[Tensor] = None, memory_key_padding_mask: Optional[Tensor] = None, pos: Optional[Tensor] = None, query_pos: Optional[Tensor] = None):
        if self.normalize_before:
            return self.forward_pre(tgt, memory, tgt_mask, memory_mask, tgt_key_padding_mask, memory_key_padding_mask, pos, query_pos)
        return self.forward_post(tgt, memory, sup_memory, tgt_mask, memory_mask, tgt_key_padding_mask, memory_key_padding_mask, pos, query_pos)
...
class Extractor(nn.Module):
    ...
    def __init__(
        ...
        self.mhsa = TransformerDecoderLayer(self.down_size, 3, self.down_size * 3, self.dropout, 'relu', normalize_before=True)
...
class TextAdapter(nn.Module):
    ...
    def __init__(self, input_size, dropout=0.1, adapter_scalar="1.0", adapter_num_layers=1, mem_adpt_self=False, SA_only=False):
        super().__init__()
                ...
                if k.startswith('visual.'):
                    ...
                    if 'positional_embedding' in new_k:
                        if self.attnpool.positional_embedding.shape != state_dict[new_k].shape:
                            ...
                            spatial_pos = F.interpolate(state_dict[new_k][
                                1:,
                            ].reshape(1, 7, 7, cls_pos.shape[1]).permute(0, 3, 1, 2), size=(H, W), mode='bilinear')
...
class ResidualAttentionBlock(nn.Module):
    ...
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, adapter: bool = False, adapter_num_layers: int = 1, multi_cross: bool = False):
        ...
        if adapter:
            self.adaptermlp = Adapter(None, d_model=d_model, dropout=0.1, bottleneck=64, init_option='lora', adapter_scalar='learnable_scalar', adapter_num_layers=adapter_num_layers, multi_cross=multi_cross)
...
class ResidualAttentionBlock_MAPLE(nn.Module):
    ...
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, adapter: bool = False, adapter_num_layers: int = 1, multi_cross: bool = False, text_layer=False, design_details=None, i=0):
        super().__init__()
        self.attn = nn.MultiheadAttention(d_model, n_head)
        self.ln_1 = LayerNorm(d_model)
        self.mlp = nn.Sequential(OrderedDict([("c_fc", nn.Linear(d_model, d_model * 4)), ("gelu", QuickGELU()), ("c_proj", nn.Linear(d_model * 4, d_model))]))
    ...
    def forward(self, x: torch.Tensor):
        ...
        if len(x) == 3:
            x, prior, text_context = x
            x, compound_prompts_deeper, counter = x
            assert self.text_layer == 0 and self.pt_begin_layer == 0

            if not self.first_layer:
                assert len(compound_prompts_deeper) > 0
                if ((self.pt_begin_layer == 0) and (not (counter > len(compound_prompts_deeper) - 1))):
                    ...
                    assert len(visual_context.shape) == 3
...
class Transformer(nn.Module):
    ...
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, adapter: bool = False, adapter_layers: List = [i for i in range(24)], adapter_num_layers: int = 1, multi_cross: bool = False, prompts_needed=0, text_layer=False, design_details=None):
        super().__init__()
        self.width = width
        self.layers = layers
        ...
        if current_trainer == 'MaPLe':
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock_MAPLE(width, heads, attn_mask, adapter=((i in adapter_layers) and adapter), adapter_num_layers=adapter_num_layers, multi_cross=multi_cross, text_layer=text_layer, design_details=design_details, i=i) for i in range(layers)])
    ...
    def forward(self, x: torch.Tensor, prior=None, text_context=None, mask=None):
        if prior is None:
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, use_adapter: bool = True, adapter_layers: List = [_ for _ in range(24)], adapter_num_layers: int = 1, multi_cross: bool = False, design_details=None):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        ...
        self.transformer = Transformer(width, layers, heads, adapter=use_adapter, adapter_layers=adapter_layers, adapter_num_layers=adapter_num_layers, multi_cross=multi_cross, design_details=design_details)
...
class VisionTransformer_MAPLE(nn.Module):
    ...
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, use_adapter: bool = True, adapter_layers: List = [_ for _ in range(24)], adapter_num_layers: int = 1, multi_cross: bool = False, design_details=None):
        ...
        self.transformer = Transformer(width, layers, heads, adapter=use_adapter, adapter_layers=adapter_layers, adapter_num_layers=adapter_num_layers, multi_cross=multi_cross, prompts_needed=self.prompt_till_layer_visual, design_details=design_details)
    ...
    def forward(self, x: torch.Tensor, prior=None):
        x, shared_ctx, compound_deeper_prompts = x
...
class CLIPTextContextEncoder(nn.Module):
    ...
    def __init__(self, context_length=13, vocab_size=49408, transformer_width=512, transformer_heads=8, transformer_layers=12, embed_dim=1024, out_dim=256, pretrained=None, **kwargs):
        super().__init__()
...
class CLIP_ResNet(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            self.visual = VisionTransformer(input_resolution=image_resolution, patch_size=vision_patch_size, width=vision_width, layers=vision_layers, heads=vision_heads, output_dim=embed_dim, use_adapter=use_adapter)

        self.text_encoder = CLIPTextContextEncoder(context_length=context_length, vocab_size=49408, transformer_width=transformer_width, transformer_heads=transformer_heads, transformer_layers=transformer_layers, embed_dim=embed_dim)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            vision_heads = vision_width // 64
            if trainer == "MaPLe":
                self.visual = VisionTransformer_MAPLE(input_resolution=image_resolution, patch_size=vision_patch_size, width=vision_width, layers=vision_layers, heads=vision_heads, output_dim=embed_dim, use_adapter=use_adapter, adapter_layers=kwargs["adapter_layers"], adapter_num_layers=kwargs["adapter_num_layers"], multi_cross=kwargs['multi_cross'], design_details=design_details)
            else:
                self.visual = VisionTransformer(input_resolution=image_resolution, patch_size=vision_patch_size, width=vision_width, layers=vision_layers, heads=vision_heads, output_dim=embed_dim, use_adapter=use_adapter, adapter_layers=kwargs["adapter_layers"], adapter_num_layers=kwargs["adapter_num_layers"], multi_cross=kwargs['multi_cross'], design_details=design_details)

        if trainer == "MaPLe":
            prompt_till_layer_text = design_details['language_depth']
            self.transformer = Transformer(width=transformer_width, layers=transformer_layers, heads=transformer_heads, attn_mask=self.build_attention_mask(), prompts_needed=prompt_till_layer_text, text_layer=True, design_details=design_details)
        else:
